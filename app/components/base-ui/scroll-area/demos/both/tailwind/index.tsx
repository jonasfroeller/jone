...
export default function ExampleScrollAreaBoth() {
  return (
    <ScrollArea.Root className="h-80 w-80 max-w-[calc(100vw-8rem)]">
      <ScrollArea.Viewport className="h-full rounded-lg border border-fd-border focus-visible:outline focus-visible:outline-fd-primary focus-visible:outline-offset-2">
        <ScrollArea.Content className="p-5">
          <ul className="m-0 grid list-none grid-cols-[repeat(10,6.25rem)] grid-rows-[repeat(10,6.25rem)] gap-3 p-0">
            {Array.from({ length: 100 }, (_, i) => (
              <li
                key={i} className="flex items-center justify-center rounded-lg bg-fd-muted text-sm font-medium text-fd-muted-foreground"
              >
                {i + 1}
              </li>
            ))}
