...
export default function ExampleAsyncAutocomplete() {
  ...
  function getStatus(): React.ReactNode | null {
    if (isPending) {
      return (
        <React.Fragment>
          <div className="size-4 rounded-full border-2 border-fd-border border-t-gray-600 animate-spin"
            aria-hidden
          />
          Searching…
        </React.Fragment>
  ...
  return (
    ...
    >
      <label className="flex flex-col gap-1 text-sm leading-5 font-medium text-fd-foreground">
        ...
        <Autocomplete.Input
          placeholder="e.g. Pulp Fiction or 1994" className="bg-[canvas] h-10 w-[16rem] md:w-[20rem] font-normal rounded-md border border-fd-border pl-3.5 text-base text-fd-foreground focus:outline-2 focus:-outline-offset-1 focus:outline-fd-primary"
        />
      </label>
      ...
      <Autocomplete.Portal hidden={!status}>
        <Autocomplete.Positioner className="outline-hidden" sideOffset={4} align="start">
          ...
          >
            <Autocomplete.Status>
              {status && (
                <div className="flex items-center gap-2 py-1 pl-4 pr-8 text-sm text-fd-muted-foreground">
                  {status}
                </div>
              )}
            </Autocomplete.Status>
