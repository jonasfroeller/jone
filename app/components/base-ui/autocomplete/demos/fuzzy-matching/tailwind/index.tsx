  ...
  return (
    ...
    >
      <label className="flex flex-col gap-1 text-sm leading-5 font-medium text-fd-foreground">
        ...
        <Autocomplete.Input
          placeholder="e.g. React" className="bg-[canvas] h-10 w-[16rem] md:w-[20rem] font-normal rounded-md border border-fd-border pl-3.5 text-base text-fd-foreground focus:outline-2 focus:-outline-offset-1 focus:outline-fd-primary"
        />
      </label>

      <Autocomplete.Portal>
        <Autocomplete.Positioner className="outline-hidden" sideOffset={4}>
          <Autocomplete.Popup className="w-[var(--anchor-width)] max-h-[min(var(--available-height),28rem)] max-w-[var(--available-width)] overflow-y-auto scroll-pt-2 scroll-pb-2 overscroll-contain rounded-md bg-[canvas] py-2 text-fd-foreground shadow-lg shadow-fd-foreground/10 outline-1 outline-fd-border">
                  ...
                  <Autocomplete.Value>
                    {(value) => (
                      <div className="flex w-full flex-col gap-1">
                        ...
                        <div className="text-sm leading-5 text-fd-muted-foreground">
...
function highlightText(text: string, query: string): React.ReactNode {
  ...
  return text.split(regex).map((part, idx) =>
    regex.test(part) ? (
      <mark key={idx} className="text-fd-primary bg-transparent font-bold">
        {part}
      </mark>
    ) : (
      part
