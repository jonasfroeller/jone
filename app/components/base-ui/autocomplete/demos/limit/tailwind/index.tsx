...
export default function ExampleAutocompleteLimit() {
  ...
  return (
    <Autocomplete.Root items={tags} value={value} onValueChange={setValue} limit={limit}>
      <label className="flex flex-col gap-1 text-sm leading-5 font-medium text-fd-foreground">
        Limit results to 8
        <Autocomplete.Input
          placeholder="e.g. component" className="bg-[canvas] h-10 w-[16rem] md:w-[20rem] font-normal rounded-md border border-fd-border pl-3.5 text-base text-fd-foreground focus:outline-2 focus:-outline-offset-1 focus:outline-fd-primary"
        />
      </label>

      <Autocomplete.Portal>
        <Autocomplete.Positioner className="outline-hidden" sideOffset={4}>
          <Autocomplete.Popup className="w-[var(--anchor-width)] max-h-[min(var(--available-height),23rem)] max-w-[var(--available-width)] overflow-y-auto scroll-pt-2 scroll-pb-2 overscroll-contain rounded-md bg-[canvas] py-2 text-fd-foreground shadow-lg shadow-fd-foreground/10 outline-1 outline-fd-border">
            <Autocomplete.Empty className="px-4 py-2 text-[0.925rem] leading-4 text-fd-muted-foreground empty:m-0 empty:p-0">
            ...
            <Autocomplete.Status className="mt-1 px-4 py-2 text-sm leading-5 text-fd-muted-foreground empty:m-0 empty:p-0">
              {moreCount > 0
