...
export default function OutsideScrollDialog() {
  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 bg-[linear-gradient(to_bottom,rgb(0_0_0/5%)_0,rgb(0_0_0/10%)_50%)] opacity-100 transition-[backdrop-filter,opacity] duration-600 ease-(--ease-out-fast) backdrop-blur-[1.5px] data-starting-style:backdrop-blur-0 data-starting-style:opacity-0 data-ending-style:backdrop-blur-0 data-ending-style:opacity-0 data-ending-style:duration-350 data-ending-style:ease-[cubic-bezier(0.375,0.015,0.545,0.455)] supports-[-webkit-touch-callout:none]:absolute" />
        <Dialog.Viewport className="group/dialog fixed inset-0">
          <ScrollArea.Root
            style={{ position: undefined }} className="h-full overscroll-contain group-data-ending-style/dialog:pointer-events-none"
          >
            <ScrollArea.Viewport className="h-full overscroll-contain group-data-ending-style/dialog:pointer-events-none">
              <ScrollArea.Content className="flex min-h-full items-center justify-center">
                <Dialog.Popup
                  ref={popupRef}
                  initialFocus={popupRef} className="outline-0 relative mx-auto my-18 w-[min(40rem,calc(100vw-2rem))] rounded-lg bg-fd-background p-8 text-fd-foreground shadow-[0_10px_64px_-10px_rgb(36_40_52/20%),0_0.25px_0_1px_oklch(12%_9%_264deg/7%)] transition-transform duration-700 ease-[cubic-bezier(0.45,1.005,0,1.005)] data-starting-style:translate-y-[100dvh] data-ending-style:translate-y-[max(100dvh,100%)] data-ending-style:duration-350 data-ending-style:ease-[cubic-bezier(0.375,0.015,0.545,0.455)] 0_0_0_1px_oklch(29%_0.75%_264deg/80%)] motion-reduce:transition-none"
                >
                  <div className="mb-4 flex items-start justify-between gap-3">
                    ...
                    <Dialog.Close
                      aria-label="Close" className="relative -top-2 -right-2 flex items-center justify-center rounded-md border border-fd-border bg-fd-background w-9 h-9 text-base font-medium text-fd-foreground select-none hover:bg-fd-muted focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-fd-primary active:bg-fd-muted"
                    >
                      <XIcon className="h-[1.1rem] w-[1.1rem]" />
                    </Dialog.Close>
                  </div>

                  <Dialog.Description className="m-0 mb-6 text-base leading-[1.6rem] text-fd-muted-foreground">
                  ...
                  <div className="mb-7 flex flex-col gap-6">
                    {CONTENT_SECTIONS.map((item) => (
                      <section key={item.title}>
                        ...
                        <p className="m-0 text-[0.95rem] leading-[1.55rem] text-fd-foreground">
                          {item.body}
                        </p>
                      </section>
                    ))}
            ...
            <ScrollArea.Scrollbar className="pointer-events-none absolute m-[0.4rem] flex w-1 justify-center rounded-[1rem] opacity-0 transition-opacity duration-250 data-scrolling:pointer-events-auto data-scrolling:opacity-100 data-scrolling:duration-75 data-scrolling:delay-[0ms] hover:pointer-events-auto hover:opacity-100 hover:duration-75 hover:delay-[0ms] md:w-1.75 group-data-ending-style/dialog:opacity-0 group-data-ending-style/dialog:duration-300">
