...
export default function InsideScrollDialog() {
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 bg-fd-foreground opacity-20 transition-opacity duration-250 ease-[cubic-bezier(0.45,1.005,0,1.005)] data-starting-style:opacity-0 data-ending-style:opacity-0 supports-[-webkit-touch-callout:none]:absolute" />
        <Dialog.Viewport className="fixed inset-0 flex items-center justify-center overflow-hidden py-6 [@media(min-height:600px)]:pb-12 [@media(min-height:600px)]:pt-8">
          <Dialog.Popup className="relative flex w-[min(40rem,calc(100vw-2rem))] max-h-full max-w-full min-h-0 flex-col overflow-hidden rounded-lg bg-fd-background p-8 text-fd-foreground shadow-[0_24px_45px_rgba(15,23,42,0.18)] outline-1 outline-fd-border transition-all duration-300 ease-[cubic-bezier(0.45,1.005,0,1.005)] data-starting-style:scale-[0.98] data-starting-style:opacity-0 data-ending-style:scale-[0.98] data-ending-style:opacity-0">
            <div className="mb-2 flex items-start justify-between gap-3">
              <Dialog.Title className="m-0 text-xl font-semibold leading-7.5">
                Dialog
              </Dialog.Title>
            </div>
            <Dialog.Description className="m-0 mb-4 text-base leading-[1.6rem] text-fd-muted-foreground">
              This layout keeps the popup fully on screen while allowing its content to scroll.
            </Dialog.Description>
            <ScrollArea.Root className="relative flex min-h-0 flex-1 overflow-hidden before:absolute before:top-0 before:h-px before:w-full before:bg-fd-accent before:content-[''] after:absolute after:bottom-0 after:h-px after:w-full after:bg-fd-accent after:content-['']">
              <ScrollArea.Viewport className="flex-1 min-h-0 overflow-y-auto overscroll-contain py-6 pr-6 pl-1 focus-visible:outline-1 focus-visible:-outline-offset-1 focus-visible:outline-fd-primary">
