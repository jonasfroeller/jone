...
export default function ExampleDrawerSwipeArea() {
  ...
  return (
    <div
      ref={setPortalContainer} className="relative min-h-[320px] w-full overflow-hidden border border-fd-border bg-fd-background text-fd-foreground"
    >
      <Drawer.Root swipeDirection="right" modal={false}>
        {/* <Drawer.SwipeArea className="absolute inset-y-0 right-0 z-10 box-border w-10 border-l-2 border-dashed border-blue-800 bg-fd-primary/10">
          <span className="pointer-events-none absolute right-0 top-1/2 mr-2 -translate-y-1/2 -rotate-90 origin-center whitespace-nowrap text-xs font-semibold tracking-[0.12em] text-fd-primary uppercase">
            Swipe here
          </span>
        </Drawer.SwipeArea> */}
        <div className="flex min-h-[320px] flex-col items-center justify-center gap-3 px-4 text-center">
          <p className="text-base text-fd-muted-foreground text-center pr-12">
          ...
          <Drawer.Viewport className="[--viewport-padding:0px] supports-[-webkit-touch-callout:none]:[--viewport-padding:0.625rem] absolute inset-0 z-20 flex items-stretch justify-end p-[var(--viewport-padding)]">
            <Drawer.Popup className="[--bleed:3rem] supports-[-webkit-touch-callout:none]:[--bleed:0px] h-full w-[calc(20rem+3rem)] max-w-[calc(100vw-3rem+3rem)] -mr-[3rem] bg-fd-background p-6 pr-[calc(1.5rem+3rem)] text-fd-foreground outline outline-1 outline-fd-border overflow-y-auto overscroll-contain touch-auto [transform:translateX(var(--drawer-swipe-movement-x))] transition-transform duration-[450ms] ease-[cubic-bezier(0.32,0.72,0,1)] data-[swiping]:select-none data-[ending-style]:[transform:translateX(calc(100%-var(--bleed)+var(--viewport-padding)+2px))] data-[starting-style]:[transform:translateX(calc(100%-var(--bleed)+var(--viewport-padding)+2px))] data-[ending-style]:duration-[calc(var(--drawer-swipe-strength)*400ms)] supports-[-webkit-touch-callout:none]:mr-0 supports-[-webkit-touch-callout:none]:w-[20rem] supports-[-webkit-touch-callout:none]:max-w-[calc(100vw-20px)] supports-[-webkit-touch-callout:none]:rounded-[10px] supports-[-webkit-touch-callout:none]:pr-6">
              <Drawer.Content className="mx-auto w-full max-w-[32rem]">
                ...
                <Drawer.Description className="mb-6 text-base text-fd-muted-foreground">
