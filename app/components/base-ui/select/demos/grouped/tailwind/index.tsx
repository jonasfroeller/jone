  ...
  return (
    <Field.Root className="flex flex-col gap-1">
      ...
      <Select.Root items={groupedProduce}>
        <Select.Trigger className="flex h-10 min-w-44 items-center justify-between gap-3 rounded-md border border-fd-border pr-3 pl-3.5 text-base bg-[canvas] text-fd-foreground select-none hover:bg-fd-muted focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-fd-primary data-[popup-open]:bg-fd-muted">
        ...
        <Select.Portal>
          <Select.Positioner className="outline-none select-none z-10" sideOffset={8}>
            <Select.Popup className="group min-w-[var(--anchor-width)] origin-[var(--transform-origin)] bg-clip-padding rounded-md bg-[canvas] text-fd-foreground shadow-lg shadow-fd-foreground/10 outline outline-1 outline-fd-border transition-[transform,scale,opacity] data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[side=none]:min-w-[calc(var(--anchor-width)+1rem)] data-[side=none]:data-[ending-style]:transition-none data-[starting-style]:scale-90 data-[starting-style]:opacity-0 data-[side=none]:data-[starting-style]:scale-100 data-[side=none]:data-[starting-style]:opacity-100 data-[side=none]:data-[starting-style]:transition-none">
                  ...
                  <React.Fragment key={group.value}>
                    <Select.Group className="block pb-0.5">
                      ...
                      {group.items.map((item) => (
                        <Select.Item
                          key={item.value}
                          value={item.value} className="grid cursor-default grid-cols-[0.75rem_1fr] items-center gap-2 py-2 pr-4 pl-2.5 text-sm leading-4 outline-none select-none group-data-[side=none]:pr-12 group-data-[side=none]:text-base group-data-[side=none]:leading-4 data-[highlighted]:relative data-[highlighted]:z-0 data-[highlighted]:text-fd-background data-[highlighted]:before:absolute data-[highlighted]:before:inset-x-1 data-[highlighted]:before:inset-y-0 data-[highlighted]:before:z-[-1] data-[highlighted]:before:rounded-sm data-[highlighted]:before:bg-fd-foreground pointer-coarse:py-2.5 pointer-coarse:text-[0.925rem]"
                        >
                    ...
                    {index < groupedProduce.length - 1 ? (
                      <Select.Separator className="my-2 mx-4 h-px bg-fd-accent" />
                    ) : null}
                  </React.Fragment>
                ))}
              </Select.List>
