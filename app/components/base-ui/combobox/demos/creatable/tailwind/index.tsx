    ...
    <React.Fragment>
      ...
      >
        <div className="max-w-112 flex flex-col gap-1">
          ...
          <Combobox.Chips className="flex flex-wrap items-center gap-0.5 rounded-md border border-fd-border px-1.5 py-1 w-64 focus-within:outline-2 focus-within:-outline-offset-1 focus-within:outline-fd-primary min-[500px]:w-[22rem]"
            ref={containerRef}
          >
            <Combobox.Value>
              {(value: LabelItem[]) => (
                <React.Fragment>
                  ...
                  <Combobox.Input
                    ...
                    id={id}
                    placeholder={value.length > 0 ? '' : 'e.g. bug'} className="min-w-12 flex-1 h-8 rounded-md border-0 bg-transparent pl-2 text-base text-fd-foreground outline-hidden"
        ...
        <Combobox.Portal>
          <Combobox.Positioner className="z-50 outline-hidden" sideOffset={4} anchor={containerRef}>
            <Combobox.Popup className="w-[var(--anchor-width)] max-h-[min(var(--available-height),24rem)] max-w-[var(--available-width)] overflow-y-auto scroll-pt-2 scroll-pb-2 overscroll-contain rounded-lg bg-[canvas] py-2 text-fd-foreground shadow-lg shadow-fd-foreground/10 outline-1 outline-fd-border">
      ...
      <Dialog.Root open={openDialog} onOpenChange={setOpenDialog}>
        <Dialog.Portal>
          <Dialog.Backdrop className="fixed inset-0 min-h-dvh bg-fd-foreground opacity-20 transition-opacity data-[starting-style]:opacity-0 data-[ending-style]:opacity-0 supports-[-webkit-touch-callout:none]:absolute" />
          <Dialog.Popup className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 mt-[-2rem] w-[24rem] max-w-[calc(100vw-3rem)] rounded-lg bg-[canvas] p-6 text-fd-foreground outline-1 outline-fd-border transition-all data-[starting-style]:opacity-0 data-[starting-style]:scale-90 data-[ending-style]:opacity-0 data-[ending-style]:scale-90"
            initialFocus={createInputRef}
          >
            <Dialog.Title className="-mt-1.5 mb-1 text-lg leading-7 tracking-[-0.0025em] font-medium">
              Create new label
            </Dialog.Title>
            <Dialog.Description className="mb-4 text-base leading-6 text-fd-muted-foreground">
              Add a new label to select.
            </Dialog.Description>
            <form onSubmit={handleCreateSubmit}>
              <input
                ref={createInputRef} className="w-full h-10 rounded-md border border-fd-border bg-[canvas] text-fd-foreground px-2.5 focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-fd-primary"
                placeholder="Label name"
                defaultValue={pendingQueryRef.current}
              />
              <div className="mt-4 flex justify-end gap-4">
                <Dialog.Close className="flex h-10 items-center justify-center rounded-md border border-fd-border bg-fd-background px-3.5 text-base font-medium text-fd-foreground select-none hover:bg-fd-muted focus-visible:-outline-offset-1 focus-visible:outline-fd-primary active:bg-fd-muted">
                  Cancel
                </Dialog.Close>
                <button
                  type="submit" className="flex h-10 items-center justify-center rounded-md border border-fd-border bg-fd-background px-3.5 text-base font-medium text-fd-foreground select-none hover:bg-fd-muted focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-fd-primary active:bg-fd-muted"
                >
                  Create
                </button>
              </div>
